...
//        protected void insert(Parser p) {
//            Parser[] newP = new Parser[parsers.length + 1];
//            newP[0] = p;
//            System.arraycopy(parsers, 0, newP, 1, parsers.length);
//            parsers = newP;
//        }
    }
    ...
    protected static class Repeat extends Element {
        ...
        protected void parse(Lexer lexer, List<ASTree> res)
            throws ParseException
        {
            //因为是重复，所以要一直匹配，直到token流不再是repeat
...
//            while (parser.match(lexer)) {
//                ASTree parse = parser.parse(lexer);
//                if (parse.getClass() != ASTree.class || parse.numChildren() > 0) {
//                    res.add(parse);
//                }
//                if (onlyOnce) {
//                    break;
...
//        protected void parse(Lexer lexer, List<ASTree> res) th{
//
//        }
    ...
    //向语法规则中添加 与pat匹配的标识符 终结符
    //与identifier的区别是什么
    public Parser token(String... pat) {
        elements.add(new Leaf(pat));
        return this;
    }
    //向语法规则中添加 未包含于抽象语法树的终结符（与pat匹配的）
    //在表达式中包含，但是在抽象语法树中不包含，比如括号
    ...
    //向语法规则中添加至少重复出现0次的语法规则p
    public Parser repeat(Parser p) {
        elements.add(new Repeat(p, false));
        return this;
    }
    //向语法规则中添加双目运算表达式（subexp是因子，operators是运算符）
